...
const Task = ({ id, title, color, onDelete, categories = [], priority, status }) => {
...
const ExtendIcon = ({ task, day, onExtendTask, currentDate, startingDay, isEndDay }) => {
  const [{ isDragging }, drag] = useDrag(() => ({
...
const TaskDetailModal = ({ task, isOpen, onClose, onSave, onDelete, currentDate }) => {
...
const TaskModal = ({ isOpen, onClose, onAddTask, onExtendTask, day, currentDate, tasks }) => {
...
const CalendarDay = ({ day, isToday, tasks, onDropTask, onExtendTask, currentDate, startingDay, onEditTask, onDeleteTask, onClick, isMobile }) => {
...
export default function Page() {
  ...
  const handleExtendTask = (task, startDay, endDate, isModifying = false) => {
    ...
    if (
      ...
      setFeedbackMessage(isModifying ?
  ...
  const getFilteredCalendarTasks = () => {
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
              ...
              <ScrollArea className="flex-1">
                <CardContent className="px-6 pb-6">
                  ...
                  <form onSubmit={addTask} className="space-y-3 mb-4">
                    ...
                    <Button type="submit" className="w-full dark:bg-blue-600 dark:hover:bg-blue-700">
                      <Plus className="mr-2 h烯-4 w-4" /> Add Task
                    </Button>
                  </form>
                  <div className="mb-4">
        ...
        <div className="flex-1 flex flex-col relative">
          {showFeedbackMessage && (
            <div className="absolute top-16 right-4 z-50 bg-green-100 dark:bg-green-900 border border-green-200 dark:border-green-700 text-green-800 dark:text-green-200 px-4 py-2 rounded-md shadow-md transition-opacity duration-300">
              {feedbackMessage}
            </div>
          )}
          ...
          <Card className="rounded-none shadow-none border-b dark:bg-gray-800 dark:border-gray-700">
            <CardHeader className="flex flex-row items-center justify-between px-4 sm:px-6 py-4">
              <div className="flex items-center flex-wrap gap-2">
                <Button variant="outline" size="icon" className="dark:bg-gray-700 dark:border-gray-600" onClick={goToToday}>
                  <CalendarIcon className="h-4 w-4" />
                </Button>
                <Button variant="outline" size="icon" className="dark:bg-gray-700 dark:border-gray-600" onClick={prevMonth}>
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <Button variant="outline" size="icon" className="dark:bg-gray-700 dark:border-gray-600" onClick={nextMonth}>
          ...
          {isMobile && (
            <Button
              className="fixed bottom-4 right-4 rounded-full h-12 w-12 flex items-center justify-center dark:bg-blue-600 dark:hover:bg-blue-700"
              onClick={() => setIsTaskModalOpen(true)}
            >
        ...
        {isMobile && (
          <Dialog open={isTaskModalOpen} onOpenChange={setIsTaskModalOpen}>
            <DialogContent className="sm:max-w-[600px] dark:bg-gray-800 dark:border-gray-700">
              ...
              <ScrollArea className="max-h-[70vh]">
                <div className="px-6 pb-6">
                  ...
                  <div className="space-y-1">
                    ...
                    {tasks.length === 0 && (
                      <div className="text-center py-6 text-gray-500 dark:text-gray-400 text-sm">
                        No tasks available. Create a task and schedule it.
                      </div>
                    )}
                    {tasks.length > 0 && filteredTasks.length === 0 && (
                      <div className="text-center py-6 text-gray-500 dark:text-gray-400 text-sm">
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setIsTaskModalOpen(false)}>Close</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        )}
